...
from wtp.actions.teardown import TeardownReport, teardown
...
def test_rm_on_adopted_worktree_removes_created_empty_dirs(tmp_path: Path) -> None:
    ...
    _new(box, adopt=True)
    _rm(box)
    assert not (path / "storage" / "framework").exists()
