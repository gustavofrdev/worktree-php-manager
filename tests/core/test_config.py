...
def test_legacy_writable_dir_becomes_list(tmp_path: Path) -> None:
    project = load_config(_write(tmp_path, 'writable_dir = "writable"')).project("x")
    assert project.writable_dirs == ("writable",)


def test_unknown_framework_is_rejected(tmp_path: Path) -> None:
    with pytest.raises(WtpError, match="framework"):
        load_config(_write(tmp_path, 'framework = "symfony"'))
