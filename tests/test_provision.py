...
from wtp.envfile import ENV_MARKER, read_env_value
from wtp.errors import WtpError
from wtp.permissions import GRANT_SCRIPT
from wtp.provision import NewOptions, ProvisionReport, provision
...
def test_composer_runs_only_when_vendor_is_missing(tmp_path: Path) -> None:
    box = make_sandbox(tmp_path)
    path = tmp_path / "Projects" / "outserv_agenda-hotfix"
    (path / "writable").mkdir(parents=True)
