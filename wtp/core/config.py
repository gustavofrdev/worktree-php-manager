...
from wtp.core.frameworks import Framework, profile_for
from wtp.core.validation import (
    ensure_domain,
    ensure_php_version,
...
class ProjectConfig:
    ...
    # codeigniter4, laravel ou vazio. Liga os cuidados específicos de cada um.
    framework: str = ""
    writable_dirs: tuple[str, ...] = ("writable",)
...
def _project_from_table(key: str, table: dict[str, Any], source: Path) -> ProjectConfig:
    ...
    checkout = ensure_safe_absolute(_expand_text(read.text("main_checkout")), "main_checkout")
    return ProjectConfig(
        key=key,
        main_checkout=Path(checkout),
        ...
        framework=_framework(read.text("framework", "")),
        writable_dirs=_writable_dirs(read, defaults.writable_dirs),
        copy_files=read.relative_list("copy_files"),
        ensure_dirs=read.relative_list("ensure_dirs"),
    )


def _framework(name: str) -> str:
    if profile_for(name) is not None or name == "":
        return name
    known = ", ".join(f.value for f in Framework if f.value)
    raise WtpError(f"framework = {name!r} não é conhecido.", hint=f"Use {known}, ou deixe vazio.")


def _writable_dirs(read: "_ProjectTable", default: tuple[str, ...]) -> tuple[str, ...]:
    # writable_dir (texto) é o formato antigo; writable_dirs (lista) é o atual.
    if "writable_dir" in read.table:
        return (read.relative("writable_dir", default[0]),)
    return read.relative_list("writable_dirs") or default
...
def _reject_bad_keys(key: str, table: dict[str, Any], source: Path) -> None:
    ...
    allowed = ({f.name for f in fields(ProjectConfig)} - {"key"}) | {"writable_dir"}
