...
from wtp.envfile import read_env_value
from wtp.fpm import php_socket, pool_identity
from wtp.http_probe import HttpProbe, HttpReply
from wtp.naming import WorktreeTarget
from wtp.permissions import pool_can_write
...
class Doctor:
    ...
    def _writable(self, target: WorktreeTarget) -> CheckResult:
        writable = target.path / target.project.writable_dir
        identity = pool_identity(target.project.php_version, self.services.php_root)
        if identity is None or not writable.exists():
            return _check(
                "writable/",
                False,
                f"sem pool do PHP {target.project.php_version} ou sem {writable}",
            )
        return _check(
            "writable/",
            pool_can_write(writable, identity),
            f"{identity.user} não grava em {writable}",
        )
