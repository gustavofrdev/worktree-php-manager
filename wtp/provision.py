...
from wtp.apache import render_vhost
from wtp.envfile import build_worktree_env, is_wtp_env
from wtp.errors import WtpError
from wtp.fpm import pool_identity
from wtp.locking import exclusive_lock
from wtp.manifest import Manifest
from wtp.naming import WorktreeTarget
from wtp.permissions import grant_group_write, pool_can_write
from wtp.runner import require_ok
from wtp.services import Services
from wtp.validation import ensure_db_name, validate_branch
...
class Provisioner:
    ...
    def _ensure_env(self, manifest: Manifest) -> None:
        ...
        env_path.write_text(build_worktree_env(source.read_text(), self._env_overrides(manifest)))
    ...
    def _env_overrides(self, manifest: Manifest) -> dict[str, str]:
        overrides = {self.project.base_url_key: self.target.url}
    ...
    def _ensure_vendor(self) -> None:
        path = self.target.path
        if not self.options.run_composer or (path / "vendor").exists():
            return
        if not (path / "composer.json").exists():
            return
        self.services.console.info("Rodando composer install (vendor/ não existe)...")
    ...
    def _ensure_writable(self) -> None:
        writable = self.target.path / self.project.writable_dir
        identity = pool_identity(self.project.php_version, self.services.php_root)
        if identity is None or not writable.exists():
            self._warn(
                f"Não consegui conferir {writable} (pool do PHP {self.project.php_version})."
            )
            return
