...
from wtp.core.envfile import is_wtp_env
...
class Teardown:
    ...
    def _remove_files(self, manifest: Manifest) -> None:
        ...
        self._remove_created_dirs(manifest)

    def _remove_created_dirs(self, manifest: Manifest) -> None:
        # Mais fundas primeiro; só pastas vazias, e só as que o wtp criou.
        root = self.target.path.resolve()
        for relative in sorted(manifest.created_dirs, key=lambda d: d.count("/"), reverse=True):
            directory = (root / relative).resolve()
            if directory != root and directory.is_relative_to(root) and directory.is_dir():
...
def _prune_empty_dirs(directory: Path) -> None:
    # Só pastas vazias saem; arquivo gravado pelo app (sessão, log) fica com a pasta dele.
